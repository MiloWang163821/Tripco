...
const startProps =
    {
        ...
        'config': { 'units': ['miles', 'kilometers'],'optimization':["none","short"]}
    };

/* Test example using a pre-defined function */
function testExample() {
    //const options = mount((
        //<Options config={startProps.config} trip={startProps.trip}/>
    //));
...
test('Check to see if table gets made correctly (Lambda)', () => {
  ...
  */
  const options = mount((   // (1)
      <Options /*updateOpts={this.updateOpts}*/ trip={startProps.trip} /*updateBasedOnResponse={this.updateBasedOnResponse}*/ config={startProps.config}  /*updateOptions={this.updateOptions}*//>
    ));
...
test('check funcs', () => {
  ...
  o.updateServer('host', 'hi')
  o.updateServer('port', 'mom')
  o.updateServer('I love', 'you')

  o.props.updateBasedOnResponse = (a) => {}
  o.connectServer()


 // o.props.config = {optimization:  ['none','short','shorter','shortest'],
   //                 units: ['miles', 'kilometers', 'nautical miles', 'user defined'] }
  o.render() //TO-DO test render
  o.toggleVisibility()
  o.state = {}
...
test('checking branches of render', () => {
  let o = new Options({})

  o.props.config = {optimization:  ['none','short','shorter','shortest'],
                    units: ['miles', 'kilometers', 'nautical miles', 'user defined'] }
  o.props.trip = {hi: false}
  o.render()

  o.props.trip = {options: []}
  o.render()

  o.props.trip = {options: {units: "miles", map: "svg", optimization: "none"}}
  o.render()
});
